...
def render(template, entry, slug):
    ...
    def repl(m):
        key, sep, quote, _ = m.group(1), m.group(2), m.group(3), m.group(4)
        field = ALIAS.get(key)
        if field is None:
            return m.group(0)
...
def append_html(slug, entries, dry):
    ...
    have = {w for w in re.findall(r"korean:\s*['\"]([^'\"]+)['\"]", text)}
...
def main():
    ...
    for p in patches:
        ...
        if os.path.exists(os.path.join(DATA, slug + '.json')):
            total += append_json(slug, p['entries'], args.dry_run, not args.no_rebuild)
        else:
            total += append_html(slug, p['entries'], args.dry_run)
    print(f'\n[+] всего слов: {total}')
