...
# Примеры со страной Россия заменяются на нейтральные. Порядок важен:
# сначала целые фразы, где простой заменой страны получилась бы неправда
# («Россия — самая большая страна»), потом отдельные слова.
COUNTRY_SWAP = [
    ('러시아가 가장 큰 나라입니다', '중국이 인구가 가장 많은 나라입니다'),
    ('Россия — самая большая страна', 'Китай — самая населённая страна'),
    ('러시아가 제일 큽니다', '중국의 인구가 제일 많습니다'),
    ('Россия — самая большая из всех стран', 'Из всех стран у Китая самое большое население'),
    ('러시아 겨울은 아주 추워요', '캐나다 겨울은 아주 추워요'),
    ('Зима в России очень холодная', 'Зима в Канаде очень холодная'),
    ('모스크바 국립 대학교', '서울대학교'),
    ('러시아 국립 인간 대학', '파리 대학교'),
    ('МГУ', 'Сеульском национальном университете'),
    # в книге фраза разорвана переносом, поэтому ловим её по частям
    ('Российском Государ- ственном Гуманитарном Университете', 'Парижском университете'),
    ('Французском Государ- ственном Гуманитарном Университете', 'Парижском университете'),
    ('Российском Государственном Гуманитарном Университете', 'Парижском университете'),
    ('Как добраться до Кремля?', 'Как добраться до дворца Кёнбоккун?'),
    ('크렘린은 어떻게 가나요?', '경복궁은 어떻게 가나요?'),
    ('크렘린', '경복궁'),
    ('Кремля', 'дворца Кёнбоккун'), ('Кремль', 'дворец Кёнбоккун'),
    ('모스크바', '파리'),
    ('러시아', '프랑스'),
    ('Москвы', 'Парижа'), ('Москве', 'Париже'), ('Москву', 'Париж'), ('Москва', 'Париж'),
    ('России', 'Франции'), ('Россию', 'Францию'), ('Россия', 'Франция'),
    ('Российск', 'Французск'),
]
...
def main():
    ...
    # Строки склеиваются из разорванных переносом кусков уже после clean(),
    # поэтому замену стран прогоняем ещё раз по готовым данным
    def swap_deep(node):
        if isinstance(node, str):
            for a, b in COUNTRY_SWAP:
                node = node.replace(a, b)
            return node
        if isinstance(node, list):
            return [swap_deep(x) for x in node]
        if isinstance(node, dict):
            return {k: swap_deep(v) for k, v in node.items()}
        return node
