    ...
    function buildQuestions(unit, exList) {
        ...
        exList.forEach(function (ex) {
            ...
            items.forEach(function (it) {
                ...
                // Порядок слов — та же сборка, что и по слогам, только плитки
                // это слова. Обманок не добавляем: лишнее слово превратило бы
                // задание на порядок в задание на выбор
                if (ex.kind === 'order') {
                    var words = answer.trim().split(/\s+/);
                    qs.push(Object.assign({}, base, words.length >= 2 && words.length <= 10
                        ? { type: 'assemble', words: true, tiles: shuffle(words.slice()) }
                        : { type: 'reveal' }));
                    return;
                }

                // Пары — в тренажёре это выбор перевода: вторая колонка урока
                // и даёт правдоподобные неверные варианты
                if (ex.kind === 'match') {
                    var pairs = pool.filter(function (v) { return norm(v) !== norm(answer); });
                    qs.push(Object.assign({}, base, pairs.length >= 3
                        ? { type: 'choice', options: shuffle([answer].concat(shuffle(pairs).slice(0, 3))) }
                        : { type: 'reveal' }));
                    return;
                }
    ...
    function start(opts) {
        ...
        function render() {
            ...
            if (q.type === 'yesno') {
                ...
                html += '<div class="' + qClass + '">' + esc(q.q) + '</div>' +
                        (q.hint ? '<div class="tr-hint">' + esc(q.hint) + '</div>' : '') +
                        '<div class="tr-slots" id="trSlots"><span class="ph">' +
                            (q.words ? 'Соберите предложение из слов' : 'Соберите ответ из слогов') +
                        '</span></div>' +
                        '<div class="tr-tiles">';
